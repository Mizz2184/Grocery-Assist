...
DROP POLICY IF EXISTS "Allow insert for authenticated users" ON public.grocery_lists;
DROP POLICY IF EXISTS "Allow insert for authenticated users" ON public.grocery_items;
DROP POLICY IF EXISTS "Allow insert for authenticated users" ON public.products;
DROP POLICY IF EXISTS "Users can view their own lists or collaborations" ON public.grocery_lists;
DROP POLICY IF EXISTS "Users can update their own lists or collaborations" ON public.grocery_lists;
DROP POLICY IF EXISTS "Only owners can delete lists" ON public.grocery_lists;
DROP POLICY IF EXISTS "Users can view items in their lists" ON public.grocery_items;
DROP POLICY IF EXISTS "Users can update items in their lists" ON public.grocery_items;
DROP POLICY IF EXISTS "Users can delete items in their lists" ON public.grocery_items;
...
-- Create function for user_payments
...
AS $$
BEGIN
  -- This function is now just a placeholder since we create the table above
  -- But we keep it for backward compatibility with the code
  RETURN TRUE;
END;
$$;
